...
A minimalist, text-first personal portfolio site. **Supabase-only** — no demo mode, no fake data. AWS deployable, Git-based, no vendor lock-in.
...
### 3. Set Bootstrap Token (IMPORTANT!)
...
After becoming admin, go to `/admin/settings` and click "Seed Demo" to populate sample projects and writing items.
...
- **Testing:** Vitest (unit tests), smoke test script, health check script
- **Deployment:** AWS (S3/CloudFront) or any static host

## Features

- **Public Site:** Home, Projects, Writing, Resume, Contact
- **Admin Dashboard:** Full CRUD for projects/writing, settings management, connection status
- **Security:** RLS on all tables, secure bootstrap mechanism
- **No Demo Mode:** Supabase is the single source of truth
...
│   └── admin/
│       ├── Dashboard.tsx  # Includes connection status
│       ├── AdminSettings.tsx
│       └── ...
docs/sql/
├── 000_all.sql          # Single idempotent setup script
└── 003_seed.sql         # Demo content seeder (admin-only RPC)
scripts/
├── smoke-test.mjs       # Page load tests
└── supabase-health.mjs  # Supabase connectivity check
```
...
| `supabase-health.mjs` | Environment vars, Supabase connectivity, schema validation |
...
| `admin_seed_demo()` | Seeds sample content (admin-only) |
...
- **No demo/fallback data** — Supabase is the only data source
...
### Public pages show "Unable to Load"
- Check if environment variables are set correctly
- Run `node scripts/supabase-health.mjs` to diagnose
- Check browser console for Supabase errors

### Admin shows "Setup Required"
- Make sure `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` are set
- Run `docs/sql/000_all.sql` in Supabase SQL Editor

### "Bootstrap token required" error
- Make sure you set the `bootstrap_token_hash` in the database (see step 3)
...
### Admin Dashboard shows connection errors
- Check the "Supabase Connection Status" card for details
